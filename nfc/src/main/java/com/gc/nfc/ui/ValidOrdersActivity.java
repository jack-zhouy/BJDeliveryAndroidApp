...
public class ValidOrdersActivity extends BaseActivity implements OnClickListener,AbsListView.OnScrollListener {
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
    }
    private SwipeRefreshLayout swipeRefreshLayout;
    public static JSONArray m_taskOrderListJson; //任务订单列表
    ...
    private void switchActivity(int position) {
        try {
            ...
            String taskId = taskOrderJson.getString("id");
            JSONObject orderJson = taskOrderJson.getJSONObject("object");

            bundle.putString("taskId", taskId);
            bundle.putInt("orderStatus", 0);//待抢单
            bundle.putString("order", orderJson.toString());

            JSONObject processJson = taskOrderJson.getJSONObject("process");
            bundle.putString("businessKey", processJson.get("buinessKey").toString());//订单号

            intent.setClass(ValidOrdersActivity.this, OrderDetailActivity.class);
            intent.putExtras(bundle);
            startActivity(intent);
        }catch (JSONException e){
            Toast.makeText(ValidOrdersActivity.this, "未知错误，异常！"+e.getMessage(),
                    ...
                    if(response!=null){
                        if(response.getStatusLine().getStatusCode()==200){
                            try {
                                ...
                                ValidOrdersActivity.m_taskOrderListJson = taskOrdersListJson;//获取任务订单列表
